  ...
  module Hiera
    class Config < Vagrant.plugin(2, :config)
      ...
      def validate(env)
        ...
        _detected_errors << ("Config path can not be empty.") if config_path.nil?
        _detected_errors << ("Config file can not be empty.") if config_file.nil?
        _detected_errors << ("Data path can not be empty.") if data_path.nil?
        _detected_errors << ("Puppet version path can not be empty.") if puppet_version.nil?
        _detected_errors << ("Puppet apt source can not be empty.") if puppet_apt_source.nil?
        _detected_errors << ("Hiera puppet version can not be empty if puppet_version < 3.0.") if install_puppet_heira? && hiera_puppet_version.nil?
        _detected_errors << ("Hiera version can not be empty.") if hiera_version.nil?
        config = File.join("#{config_path}", "#{config_file}")
        _detected_errors << ("Config file not found at '#{config}'.") unless File.exists?(config)
        _detected_errors << ("Data directory not found at '#{data_path}'.") unless File.exists?("#{data_path}")
      end
    end
  end
end
